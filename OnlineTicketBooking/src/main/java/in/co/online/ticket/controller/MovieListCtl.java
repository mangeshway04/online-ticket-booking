...
/**
 * Servlet implementation class MovieListCtl
 */
@WebServlet(name = "MovieListCtl", urlPatterns = { "/ctl/MovieListCtl" })
public class MovieListCtl extends BaseCtl {
	
	private static final long serialVersionUID = 1L;
       
	private static Logger log = Logger.getLogger(MovieListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			long mId=DataUtility.getLong(request.getParameter("mId"));
			if(mId>0) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				MovieBean deletebean = new MovieBean();
				for (String id : ids) {
	...
	protected String getView() {
		...
		return OTBView.MOVIE_LIST_VIEW;
	}
